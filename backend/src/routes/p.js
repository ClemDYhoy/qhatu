// C:\qhatu\backend\src\routes\products.js
...
// ===== NUEVO ENDPOINT: Estadísticas de productos por categoría =====
router.get('/stats-by-category', async (req, res) => {
  try {
    ...
    productCounts.forEach(count => {
      countsMap[count.categoria_id] = {
        total: parseInt(count.total_productos) || 0,
        disponibles: parseInt(count.disponibles) || 0
      };
    });

    // Categorías principales con sus conteos incluyendo subcategorías
    const mainCategories = categories
      ...
      .map(cat => {
        ...
        // Sumar productos de categoría principal y subcategorías
      ...
      .filter(cat => cat.total_productos > 0); // Solo categorías con productos
